...
- **Privacy / local-first** — legal/job/health material; local-first like Reframe and PMA, not a hosted service. Likely a hard guard.
...
1. **Adopt / adapt / build — do this first.** Evaluate existing ND tools through the crip-time + output-format-bias lens before committing to build-from-scratch. June named **Goblin Tools** (magic to-do + compiler — brain-dump→steps, which she wants) and **Taskmaster**; also **Tiimo** (autism/ADHD-designed). Likely shape: *adapt a brain-dump/breakdown engine, build the goal-alignment + relational-project + crip-time layer around it* (off-the-shelf tools won't carry the guards or the `reaching_for` layer). Sidesteps the build-it-all-yourself dichotomy.
...
5. **Capacity-axis set** — port concretely from June's sister's framework (+ her own).
6. **Learning-loop design** — own session; duration-estimation first.

## D. Research to conduct
- **Existing ND tools, for the adopt/adapt/build eval** (§C.1): Goblin Tools, Taskmaster, Tiimo, Amazing Marvin, Sunsama — through the crip-time + output-format-bias lens.
