...
| `horizon` | select | **Chapter / Ongoing / Milestone.** Chapter = a life phase ending when a condition is met (job search, recovery season). Ongoing = structural, no discrete end (health, craft practice, building systems). Milestone = discrete done-state (publish paper, launch tool). Replaces the vague Long/Medium/Short-term labels. Feeds Goal ordering; the AI reads it to choose narration register (urgency vs. spiral vs. open horizon). |
| `resolution_condition` | text | For Chapter goals: what this goal looks like when it stops claiming attention. "Until income covers basic needs and the GA decision is made." Optional on Ongoing/Milestone goals but useful when the done-state needs spelling out. |
...
| `goal_engagement` | select | **Steady / Open / Backburner — added 2026-07-13, `scripts/build_goal.py`.** The Goal-level analog of a Project's `engagement`: is this whole life-area currently getting focused energy (rare — most goals sit at Steady or Open), ongoing baseline attention (`Steady`), a genuinely open field of many possible projects engaged flexibly rather than one thing to steadily feed (`Open`), or set aside (`Backburner`)? **No `Sprint` at this level** — a sprint is a Focus Period foreground state on a *project*, not a property of a whole life area (June, 2026-07-13). Feeds the not-yet-built cross-goal balancing/route-prioritization engine (§8d); until that's built, this is data worth keeping current, not yet consumed by code. Was previously unpopulated on 6 of 7 goals because no goal-creation path ever set it — fixed 2026-07-13. |
...
| `engagement` | select | **Steady / Open / Needs Clarifying / Backburner / Done.** How June is currently engaging with this project. Drives daily-plan surfacing: Steady → a normal daily move the plan commits to; Open → available, surfaced gently, not pushed (the default a new project is born with); Backburner → not surfaced unless a neglect threshold; Needs Clarifying → flagged; Done → excluded. **`Sprint` and `Hyperfixation` were RETIRED 2026-07-16** (`docs/spec_reconciliation_selection_2026-07-11.md`): a sprint is a Focus Period *foreground* action, not a stored engagement; hyperfixation is a qualitative/affective state carried in `engagement_notes` / `Affective`, not a select value. Replaces the old `Project status` (Active/Parked/Inactive) — legacy. **Learning loop target**: system learns what each level implies for each project from June's corrections. |
...
| `is_workstream` | checkbox | **Added 2026-07-13.** Marks a project as an internal dev/research *workstream* (e.g. GRA's "Metabolize Session 3," or Controlled Drift's own "Route prioritization and task breakdown") rather than a real project/subproject — see `docs/map_design.md` "Workstreams are never rendered as projects/subprojects" for the full convention. Set on the workstream-grain project itself, not on the container that holds it; unset descendants inherit `true` from the nearest ancestor that has it set (same walk-up-the-chain mechanism as `side`, `scripts/daily_plan.py`). **Every UI surface that renders projects/subprojects must render a workstream as structurally distinct, never inline in the same format** — this is a standing convention, not a one-surface fix. As of 2026-07-13 the data is tagged; the renderer code (`orient_map.py`, `review_surface.py`/`surface_template.html`) has not yet been updated to act on it. |
...
| `status` | select | **Ready / In Design / Parked / Needs Clarifying / Blocked / Done.** `Ready` = well-specified, executable now. `In Design` = real committed work but needs a design conversation before execution. `Parked` = captured, no work stream home yet, intentionally held — not surfaced in daily plan or orient map; appears in startup count; triaged via weed-park pass. `Needs Clarifying` = shape unclear; weeding gate output for vague items. `Blocked` surfaces `blocked_on`. "Active tasks" are derived: tasks linked to streams where Engagement ∈ {Steady, Sprint, Hyperfixation} with status = Ready. (Legacy: tasks created before the Active→Ready rename may carry `Active`; treated identically to `Ready` in queries.) |
...
#### Focus Period *(custom Anytype type — the short-term configuration June authors)*

<!-- Added to this spec 2026-07-18. The type has been LIVE since 2026-07-02 (`ae813c9`) with 7 real
objects, but its design reasoning was never migrated here — it lived only in
`docs/focus_configuration_addendum.md` (session 23, 2026-07-01), the build plan
`docs/superpowers/plans/2026-07-02-focus-configuration-layer.md`, and `scripts/build_focus_period.py`.
Every row below is recovered from those sources and cites them; nothing here is newly authored. -->

**The problem it fixes.** June's daily plan didn't line up with her goals, because her goals *rotate* — what's in front changes week to week, driven by events the system can't predict (a deadline, a recovery stretch, caregiving). The system held her tasks but had **nowhere to hold which goals are in front right now**, so the plan drew from the whole pile. The fix is not per-project toggling (too granular) and not an automatic cross-project ranking (the imposition June refuses). It is **a third thing that makes both downstream: a configuration June authors.** She says in plain terms what this stretch of time is about; the plan is built from it. **She stays the author of what matters — never the machine.** (`docs/focus_configuration_addendum.md`, "Read this first" + "The core move.")

**Two layers, and the period wins.** Some context is *enduring* and lives on the Project (`engagement` + `engagement_notes` — "cuffs are worth keeping a little going regularly"). Some is *short-term* and lives here: essentially a calendar plus a set of overrides. **When the short-term period contradicts the enduring baseline, the period wins** (June's decision); the project holds the default, the period overrides it while active, then the default returns. (Addendum, "Two layers … precedence.")

**Where the nuance lives (load-bearing).** The system does **not** meet June's variability by predicting circumstances and adding a field for each — *that* would be the rigid thing. It meets it two ways: rich free text read by a capable model, and a plan output allowed to change shape. **The fields are deliberately minimal; the intelligence is in the reading, not the schema.** Rule of thumb: *if we find ourselves adding a field to capture a feeling or a nuance, that is the signal we are doing it wrong* — it belongs in the free text. Each structured knob is the **instruction Python executes**; the free-text note is the **meaning the LLM reads**. Non-determinism is confined to the free text, where it belongs. (Addendum, "Where the nuance actually lives.")

**Not a "reduction."** June's catch: "reduced availability" imposes a frame that won't fit real life — a *sprint* week is *more*, not less. The object records **overrides**, neutral statements of how *this* period should be planned. That is why the fields are named `availability_*` rather than `reduced_availability_*`. (Addendum, "Reframe.")

| Field | Type | Notes |
|---|---|---|
| `name` | text | E.g. "Week of Jun 30 — jobs foreground". |
| `period_start` / `period_end` | date | When this stretch begins and lapses. The end date is what triggers the re-author prompt when the configuration goes stale — the exact failure this type fixes. Lapsed periods are **kept, not deleted**: history is a free win for the learning loops and a record of how June's phases actually went. |
| `intent` | text | **Free text: what this period is about, in June's words.** Read by the LLM as framing, never reworded by the generator. This is the primary carrier — see "Where the nuance lives" above. |
| `availability_start` / `availability_end` | date | A *narrower* free window inside the period (caregiving days, mornings only). Empty = the whole period is available. Structured so Python can deterministically test "is today in the window" without parsing natural language (`focus_period.in_availability_window`). Feeds the output-shape choice. |
| `availability_note` | text | **The situated meaning of that window** — "caregiving; fragmented time; survival-first in whatever windows exist; no deep-focus blocks." The dates only tell Python *when*; this text is *what it means*, and it is what the planner actually reads. June: "this note IS the mechanism… the anti-flattening path." Never compressed into the dates. A period may describe as many availability shapes as it needs here; the single structured window is only for Python's is-today-affected test. |
| `days_off` / `days_on` | text (structured date list) | Per-day overrides of the weekly default (`Sat`/`Sun`, from `settings.json`). `days_off` forces a day off; `days_on` forces a work day. **Constraint: structured, not natural language, so Python tests it deterministically** — honors the regex-burn lesson. Precedence in `focus_period.is_day_off`: an explicit `days_on` date wins, then `days_off`, then the weekly default. Breaking the planner's hidden assumption that every day is a full work day was the point — a wrong default for June. A malformed list must surface to June, never be silently ignored: a silently-dropped day off that plans a workday is quietly punitive. |
| `output_format` | select | **Auto / Clock schedule / Priority list.** The plan's rendered shape. The clock-time schedule literally cannot express "a couple of hours, whenever they come" — forcing it is an instance of June's own output-format-bias principle. So a fragmented or unpredictable stretch gets a **priority-ordered short list to pull from when a window opens**: few items, plain language, no clock pressure. `Auto` resolves from structured inputs only (`focus_period.resolve_output_shape`): priority iff today is in the availability window. The capacity nuance is the model's to read from free text, not Python's to scan. |
| `workday_end` | text `HH:MM` | The day's end bound. Designed to be pushed **wider** for a sprint (working till 10, 11, midnight) or narrower for a gentle week — June asked for this explicitly. Empty = the system default (18:00). |
| `foreground_projects` | objects-relation | Which projects are in front this period, overriding their enduring `engagement`. **Drives real selection by id**, not a prose hint to the LLM. |
| `paused_projects` | objects-relation | Projects held out of plan generation while the period is active. **Pausing must be rare — explicit-stop only.** The first LLM authoring run over-paused (pausing every non-foreground project, which *drops* their tasks). Corrected: non-foreground ≠ paused; non-foreground projects stay present, just backgrounded. Default `[]`. |

**How June authors it.** Same low-friction path as the log tab: she *says it* (voice or chat), the AI structures it into these fields and reflects it back, she confirms or edits. **She never fills a form.** Python anchors the date so "Saturday" resolves to a real calendar date — the agent must never infer the date or weekday, it cannot do it reliably. (Addendum, "How June inputs it" + "Deterministic Python resolves every time/calendar fact.")

**Facts travel with their meaning, always.** "Caregiving period active" is useless to the planner without "= fragmented focus, survival-first in available windows, no deep-focus blocks." So the bundle sent to the LLM pairs each computed fact with the relevant free text: `availability_note`, `intent`, the project's `engagement_notes`, the goal's `reaching_for`. Same principle as everywhere in this spec — the structured field carries the *fact*, the free text carries *what it means for June right now*, and the meaning is never flattened into the fact.

**Two sides stay structurally separate.** A period never mixes obligation-side and wellbeing-side surfacing (see Project `side`). Wellbeing-side hobby work that is non-foreground should render as an **open, self-directed block June fills from the map** — not specific tasks the system picks. June: "hobbies should be framed as open-ended scheduling blocks, so I get to choose myself — this is better than the system choosing for me, poorly." The system may surface a neglected *obligation*; it must **never** nag about a neglected hobby. (Plan, "Learned from first real use," `2a85dbd`.)

> ⚠ **Recovered design vs. the live type — recorded, not reconciled (2026-07-18).**
> 1. **`workday_start` does not exist.** `docs/review_reorganize_backend_spec.md` §17 specifies `workday_start` + `workday_end` and flags the start as **NEW** ("the object previously had only an end"). The live type has **only `Workday end`**, and the scheduler's day-bounds logic is still end-only. Unbuilt, not dropped.
> 2. **`days_on` is spec-OPEN but live.** Backend spec §17 says `days_on` "is stubbed in the UI but not yet a committed field — leave OPEN." It **is** a committed field in `build_focus_period.py`, is parsed by `focus_period.py`, and is populated on 4 of the 7 live objects. The spec text is stale.
> 3. **Date-list encoding disagrees between docs and code.** The build plan's Task 2.1 tests a JSON array (`'["2026-07-03"]'`); `focus_period_adapter._csv` writes **comma-separated** and all live values are comma-separated. `focus_period._parse_date_list` accepts both, so nothing is broken — but the plan's stated shape is not the one in the data.
> 4. **`availability_*` is used only in the "less" direction.** The addendum explicitly reframed these as neutral overrides because "a sprint week is *more*, not less." All 4 live notes describe reduced or fragmented availability, and `workday_end` — the field that carries the *widening* case — is unpopulated on all 7. The "a period can be MORE" half of the reframe has no live use yet.
> 5. **Periods are day-scale in practice, week-scale in the design.** The addendum and the field names assume a week-long stretch with a lapse-and-re-author cycle; 4 of the 7 live objects have `period_start == period_end`, and one named "Week of Jul 14" is a single day. Whether the design or the usage should move is June's call.
...
### 8h. Directory binding + Orient function *(design complete, session 10, 2026-06-19)*

**The problem this solves:** the drift skill is global but location-blind. Invoked from the job-search folder it reaches Anytype, but doesn't know that folder *is* the Material survival cluster. The bind mechanism ties a folder to a slice of the Anytype graph so that *being there* auto-contextualizes the system — capture arrives already linked to the right Goal/Project, orient shows that cluster's structure first, planning scopes to the bound project. The folder carries context; Anytype remains the one global store.

**Marker file:** `.gsdot` (JSON). Name decided session 7 — June rejected `.drift` (the bare word reads as stressful stripped of the "controlled" reframe). Format: `{"system": "Controlled Drift", "binds_to": {"goal": {"id": "...", "name": "..."}, "projects": [{"id": "...", "name": "..."}]}}`. No `mode` field — dropped. The bound project's identity does what mode was trying to do; a separate mode flag is redundant and adds cognitive overhead.

**Resolution order (three cases, all surface as offers — never silent adoption):**
1. `.gsdot` here → offer: *"This folder is connected to [Project name]. Work from that context, or treat this as a general session?"*
2. No `.gsdot` here, parent has one → offer: *"This folder doesn't have a `.gsdot`. Should I work from [parent folder]'s context, create a new project here, or treat this as a general session?"*
3. Nothing found → no binding is active; proceed exactly as today, with the full Anytype space in context.

Ignore-list and pattern-triggered offers are deferred to the proactive/learning layer (post-v1).

**On startup in any directory:** always check for a `.gsdot` file, and if not found, walk up the directory tree. This check is fast and determines whether the session is context-scoped or global. Don't skip it.

**Skill behavior when bound:** silent until consequential. The startup offer (above) is the surface — no redundant banner on every session. The binding shapes behavior quietly: weeding alignment starts from the bound Goal/Project and its related items, planning scopes to it, capture suggests linking to the bound project by default. The binding is a *suggestion to the instance*, not a mandate — June can always route differently.

**Sub-project hierarchy (already built — rendering gap only):** `parent_project` (a link field pointing to another Project) exists on the Project type and is live in Anytype. The data model is correct. What's missing is hierarchy-aware *rendering* in the scripts, skill, and prompts — they currently load Projects as a flat list and lose the nesting. Fix: any Project with a parent project set is labeled "sub-project" in display; the weeding gate, daily plan, and skill all render the hierarchy as a tree (Goal > Project > Sub-project > Tasks), never a flat list.

**Filesystem hierarchy maps to project hierarchy:** a parent `.gsdot` detected by walk-up auto-establishes the `parent_project` link when creating a new binding — no question asked. If you're in a sub-folder of a bound repo, and you create a `.gsdot` there, the new Project is automatically a child of the parent's bound Project. Cheap, natural, consistent with how the filesystem already communicates containment.

**Binding creation flow (conversational — not a CLI command):** triggered when June chooses "create a new project here" from the resolution offer. The skill runs:
1. Confirms the project name (directory name is the default: *"I'll call this [dirname] — right?"*)
2. Asks which Goal it belongs to (shows existing Goals to pick from)
3. Parent `.gsdot` detected automatically — parent_project link set without asking
4. Calls `init_binding()` (already in `scripts/gsdt_bind.py`), confirms with read-back + ding

First bindings: this repo (controlled-drift root) → *"Build Controlled Drift"* Project; job-search folder → *Material survival* Goal. Both at top level (not sub-projects).

---

**Orient — fifth skill function** (alongside capture / weed / plan / stuck).

**What Orient is for:** June loses track of the larger structure of complex projects — what the threads are, how the pieces relate, what's the current-sprint work vs. what comes later. She can't hold that picture internally. Orient makes it visible, on demand, without requiring her to reconstruct it from memory. It is an externalized cognitive structure, not a productivity feature.

**Core ADHD accessibility parameter (load-bearing, not cosmetic):** the grouping of work into named routes or streams is the accessibility mechanism — not the listing of tasks. A flat list of 50 todos is cognitively inaccessible; the same 50 todos organized into 4 named streams (e.g. "v1 pipeline," "binding mechanism," "future increments," "maintenance") is navigable because June can see what belongs together without holding the relationships in her head. This governs how Orient presents output at every mode: never a flat list; always structured by route/stream.

**The sweep does not find todos already in Anytype.** The read-only mode surfaces what's already in Anytype. The sweep mode finds what's in the repo that isn't captured yet. These are distinct and must not be conflated — conflating them would either resurface already-captured items as "new" (noise) or skip the repo content that hasn't been captured (gap).

**Three modes (the skill infers from what June says — she never names a mode):**

*Mode 1 — read-only orient ("show me the structure"):*
Reads Anytype only. Renders the existing hierarchy — the bound Project, its sub-projects, tasks nested under each — as a navigable tree. Can be discussed and talked through at length. Nothing changes in Anytype. Triggered by: "what's the structure here," "show me what's in the system," "where are we," "orient me" (when no sweep is implied). This is also the entry point for talking through the structure without doing anything else.

*Mode 2 — additive sweep ("sweep this" / "orient me" with new content):*
Reads the repo (ROADMAP, planning docs, TODO files, task-like signals in code) AND loads what's already in Anytype for the bound project. Groups new findings into named routes/streams *against the existing structure* — additive, builds on what's there, does not replace it. Proposes: sub-projects to add, tasks under each, context text for the Project. Requires June's confirmation before anything is created in Anytype — she sees the full proposed structure first. After confirmation: creates new sub-projects and tasks, links existing ones, writes/updates the `.gsdot` if needed, dings. Triggered by: "sweep this," "I've lost the thread," "add these to the system," or the initial binding creation flow (which is Mode 2 with no existing structure).

*Mode 3 — reorganization (parked — see §10):*
Deeper restructure of existing organization. Never automatic. Agent may *suggest* it when the current structure is genuinely getting in the way — offered once, as a soft suggestion June can accept or ignore — but nothing is reorganized until she explicitly confirms. Full design deferred.
...
- **Two-tier learning loop: signal detection + response calibration (June, 2026-06-16).** Two distinct loops needed. (1) *Signal precision:* affect detection is multi-signal (typo density is one heuristic; others will surface in use); the loop gets more precise over time about which signals correlate with which states. (2) *Response calibration:* what to DO about a detected state can't be predicted upfront and must be learned — counterintuitive responses are the norm (overwhelm for June → longer list of completable checkable items, not shorter, because visible progress reduces anxiety while a sparse list increases it). Both loops surface patterns to June for confirmation rather than auto-applying. Design pass once there's real use data.
  ...
  **Qualitative patterns resist the current EMA architecture (June, 2026-06-18 — add to two-tier loop design).** The meal-timing learning loop uses an 80/20 exponential moving average against a single number (lunch hour). That works for clock-time drift because the signal is unambiguous and compresses cleanly. But most of what the system needs to learn is *qualitative and multi-variable*: "I need 20 min of low-stakes tasks before I can start deep work," "errand batching works but only when energy is above a certain floor," "SSRC always gets pushed to later when EF is low." These patterns don't compress to a single number — they're contextual, co-determined by multiple signals, and can only be read across multiple sessions. The raw material is already being captured: the corrections log records proposed-vs-actual for every plan (what the system suggested, what June actually did). What's missing is the *interpretation layer* that reads across those corrections for candidate patterns and surfaces them to June as proposed Strategy objects for her to confirm or reject — not auto-applied. This is the link between the corrections log (already running) and the `Strategy` type (§2): detected behavioral patterns → proposed strategies → June confirms → system applies. The two-tier loop design session must address this qualitative dimension explicitly; a purely numerical/EMA approach will flatten the most important patterns.
...
- **Repo/folder ↔ Goal/Project binding + Orient function — DESIGN COMPLETE (session 10, 2026-06-19). See §8h.** Spike in `scripts/gsdt_bind.py` (labeled NOT WIRED) is ready to wire. Build items: (1) wire the conversational creation flow into the skill; (2) add hierarchy-aware rendering to `daily_plan.py`, weeding gate, and SKILL.md; (3) write the first two `.gsdot` files (this repo → "Build Controlled Drift"; job-search folder → Material survival); (4) add Orient (Mode 1 + Mode 2) as a fifth skill function in SKILL.md.
- **Orient Mode 3 — deep reorganization (parked, session 10, 2026-06-19).** When the existing sub-project structure becomes genuinely misleading — wrong groupings, routes that have merged, threads that have split — a deeper restructure pass would be valuable. Design not yet done. Guard for now: the agent may *suggest* reorganization when it would produce a meaningfully better ADHD surface than the current form ("I notice the structure might be clearer if..."), but only when it genuinely adds value, offered once as a soft suggestion June can accept or ignore. Never automatic. June accepts or ignores. The Mode 1 read-only orient + Mode 2 additive sweep may make this unnecessary in practice — worth checking before designing the full flow. When designed: it should let June see the proposed new structure, compare it to the old one, and confirm before anything changes.
