...
| `engagement` | select | **Steady / Sprint / Backburner.** Same vocabulary as Project engagement. Steady = working on it bit by bit, not sprinting. Sprint = concentrated push right now. Backburner = alive but not prioritizing. Drives multi-goal balancing: Sprint goals get priority framing in the daily plan; Backburner goals are shown for alignment context but de-emphasized. |
