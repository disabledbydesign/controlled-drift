...
**The problem this solves:** the drift skill is global but location-blind. Invoked from the job-search folder it reaches Anytype, but doesn't know that folder *is* the Material survival cluster. The bind mechanism ties a folder to a slice of the graph so that *being there* auto-contextualizes the system — capture lands pre-aligned, orient starts from the right cluster, planning scopes to the bound project. This is the inverse of `relevant_docs`: that points a Project outward to files the AI reads; this points a folder inward to the part of the graph it belongs to.
...
3. Nothing found → silent global-unbound; proceed exactly as today.

Ignore-list and pattern-triggered offers are deferred to the proactive/learning layer (post-v1).

**Skill behavior when bound:** silent until consequential. The startup offer (above) is the surface — no redundant banner on every session. The binding shapes behavior quietly: weeding alignment starts from the bound cluster, planning scopes to it, capture pre-suggests a link to the bound project. The binding is a *suggestion to the instance*, not a mandate.

**Sub-project hierarchy (already built — rendering gap only):** `parent_project` (objects-relation) exists on the Project type and is live in Anytype. The data model is correct. What's missing is hierarchy-aware *rendering* in the scripts, skill, and prompts — they currently load Projects as a flat list and lose the nesting. Fix: any Project with a `parent_project` is labeled "sub-project" in display; the weeding gate, daily plan, and skill all render the hierarchy as a tree (Goal > Project > Sub-project > Tasks), not a flat list.
...
**Core ADHD accessibility parameter (load-bearing, not cosmetic):** grouping into routes/streams is the accessibility mechanism. A flat list of 50 todos is cognitively inaccessible. The same 50 todos organized into 4 named routes is navigable — June can see the larger structure (what the threads are, how pieces relate, what the v1 path is vs. what's later) without having to hold it internally. This governs how Orient presents output at every mode: never a flat list; always structured by route. Orient is not a productivity tool — it is an externalized cognitive structure June can't hold herself.
...
- **Orient Mode 3 — deep reorganization (parked, session 10, 2026-06-19).** When the existing sub-project structure becomes genuinely misleading — wrong groupings, routes that have merged, threads that have split — a deeper restructure pass would be valuable. Design not yet done. Guard for now: the agent may *suggest* reorganization when it would produce a meaningfully better ADHD surface than the current form ("I notice the structure might be clearer if..."), but only when it genuinely adds value, in permission-granting register, offered once. Never automatic. June accepts or ignores. The Mode 1 read-only orient + Mode 2 additive sweep may make this unnecessary in practice — worth checking before designing the full flow. When designed: it should let June see the proposed new structure, compare it to the old one, and confirm before anything changes.
