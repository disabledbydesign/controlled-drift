...
- **Repo/folder ↔ Goal/Project binding (June, session 7 — requested).** The `drift` skill is global but *location-blind*: invoked from the job-search folder it works "from anywhere" yet doesn't know that folder **is** the Material survival cluster. The ask: bind a repo/folder to a specific Goal/Project so that *being there* auto-contextualizes capture/weed/plan to that slice of the graph — capture lands pre-aligned, the weeding alignment step starts from the right cluster, planning is scoped. (This is the **inverse** of `relevant_docs`: that points a Project/Task *outward* to files the AI reads; this points a *folder inward* to the part of the graph it belongs to.) **Proposed mechanism** (parallels the `.reframe-active` dotfile pattern June already uses): a small per-repo marker file (name TBD — **not** `.drift`; June, session 7: the bare word "drift" reads as stressful stripped of the "controlled" reframe; candidates incl. `.anchor` / `.aligned`) (or a stanza in that repo's own `CLAUDE.md`) — naming the Anytype Goal/Project this location maps to. An instance starting in a bound repo reads the marker → loads that slice → operates pre-contextualized. Keeps the system **global** (Anytype is still the one store); location merely carries context. **First binding:** the job-search folder → *Material survival* Goal. **Open sub-questions** (decide at design): id-vs-name binding (ids stable, names legible — likely store id, show name); one folder → many Projects (a repo spanning routes); and **precedence in a *mixed* repo** — the job-search folder is pure-Controlled-Drift, but a code repo has its own dev-tasks, so a bound code repo needs a rule for when "todos here" means GSDO vs. local dev work (the job-search case sidesteps this; design the precedence when the first mixed repo needs it). Cheap to prototype (read a dotfile on session start).
