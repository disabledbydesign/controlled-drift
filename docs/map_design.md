...
## The phone Map tab — planned redesign into a navigator (v1.5, scoped 2026-07-13)

The June-facing **Map tab** in `docs/overlay_daily.html` (`#tab-map`) currently dumps `orient_map.render_map()`'s 66-column monospace text into a `<pre>` blob that horizontal-scrolls on the 360px phone. It is slated to become a **drill-in navigator**: tap a project → its subprojects → its children, breadcrumb back, a leaf subproject opening to its task arc (the same `✓ · here · ☐` arc). This is the home for navigating the **deeply-nested** projects (GRA ~20+ subprojects, Build-my-own-business, Build Controlled Drift) that a daily-plan chunk deliberately does not expand. Page-by-page descent, not an accordion. **Blocked on a subproject/workstream data-cleanup pass; not v1.** Full rationale + the daily-plan side of the split: `display_grain_design.md` §9.
