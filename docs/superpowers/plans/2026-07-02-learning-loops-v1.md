...
## REVIEW 2026-07-12 — against the wiring audits; read before building

This plan was fully written and critic-swarm-revised on 2026-07-03 but **never built**. Since then, two days of wiring audits and a duration-intelligence build (committed today) changed the ground under three of its five tasks. June conditioned launching this plan on exactly this review. Bottom line up front: **launch with adaptations, do not build as-is** — one task already shipped elsewhere today, one needs a mechanical re-point onto a file that changed under it, and the plan's core "log now, interpret later" stance is *confirmed correct* by the audits (so no redesign).

### What changed since 2026-07-03 (the findings this plan predates)

- **Duration-intelligence shipped today.** `docs/superpowers/plans/2026-07-12-duration-intelligence.md` was not just written but **built and committed today** (commits `089b9ef` schema, `1356475` seam, `38d5fc2` estimate, `4a97fc7` memory-pass, `2842c00` backfill). It absorbed this plan's Task 1 outright, and added a `Duration source` label (`stated`/`estimated`) that now feeds this plan's Task 4. Verified live in code: `capture_fields.build_optional_props` writes both `Duration min` and `Duration source`; `capture_generate.py`'s contract emits `duration_min` (June's word) *and* `duration_estimate_min` (the model's guess) as separate fields.
- **Eight distinct log formats now exist** (enumerated below). This plan reads meaningfully from exactly **one** of them (`generation_log`, in Task 2). It pays no heterogeneous-format integration cost — that cost is real but lands on the *deferred interpretation layer*, not on this logging pass.
- **`plan_corrections.jsonl` has been silent since 2026-06-30.** The negotiate code path is intact (`plan_generate.py:885` still calls `log_correction`); the signal died because June stopped pressing the negotiate button, not because anything broke. It has **not** resumed after today's builds (last entry `2026-06-30T09:11:58`). No task in this plan consumes it — Task 3 explicitly distinguishes itself from it — so this is context, not a blocker.
- **Mark-done still produces no learning signal.** `task_actions.complete_task` writes `Task status=Done` to Anytype with read-back, but appends to **no** log. There is no durable completion-event record anywhere. This doesn't block any task here (Task 5's interpretation half, which would have needed it, was already cut) — but it is the missing input the deferred interpretation loops will hit first.
- **The staleness/surfacing resolver moved into `surface_log.py`** today (commit `3468499`), as a shared helper below the two `log_surfaced` functions. Those two functions — the ones Task 5 edits — are unchanged and still match Task 5's "before" block; the edit is still applicable, just now surrounded by new code (see Task 5 verdict).
- **Surfacing volume is way down per generation** (the selection gate: ~12–13 items surfaced per plan vs. the old ~82). Checked against `surface_log.jsonl`: **no task in this plan has volume-dependent math or thresholds** — Task 2 groups by name, Task 4 is an EMA, Task 3 is a diff, Task 5 is an append. The volume change is immaterial to every task's correctness. (Task 2's digest is *inherently* thin — only 24 capture records exist across all time — but that's unrelated to the selection gate and doesn't affect correctness; it's a report, thin-but-right.)

### The eight log formats (writer → shape)

1. `generation_log.jsonl` — `generation_log.py` — `{backend, duration_s, source, success, ts, structural{...}, error_*}` — **the one this plan reads** (Task 2, via `structural.dedup_notes`).
2. `surface_log.jsonl` — `surface_log.py` — `{ts, plan_date, id, name, type}` — **the one Task 5 writes to** (adds `capacity`).
3. `plan_corrections.jsonl` — `plan_corrections_log.py` — `{ts, kind, before, after}` — silent since 06-30; unused here.
4. `plan_snapshots.jsonl` — `plan_snapshot_log.py` — `{ts, plan_date, source, plan}` — a future Task-4 input, not read today.
5. `signal_log.jsonl` — `signal_log.py` — `{ts, raw, source, reference}` — a future Task-4 input (`source="log_day"`), not read today.
6. `status_check_log.jsonl` — `status_check.py` — `{ts, event, findings, ...}` — unused here.
7. `memory_pass_log.jsonl` — `memory_pass_log.py` — `{outcome, entry, candidate, anytype_id, error}` — unused here.
8. `session_log.json` — `session_store.py` — nested `{streams:{capture, negotiate}}` with a loud failures channel (`log_failure`) — unused here.

**Integration-cost verdict:** the plan does **not** assume fewer formats — it deliberately touches one at a time (each task reads/writes a single format). So **a log-format consolidation should NOT precede this build.** It *should* precede the deferred interpretation layer (Task 4's Phase 9, the cut Task-5 interpretation), which will have to join `signal_log` + `plan_snapshots` + `surface_log` + a not-yet-existing completion log. Recommend a small consolidation — and a completion-event log — as the first move of *that* future pass, not this one.

### Per-task verdict

| Task | Verdict | Why |
|---|---|---|
| **1 — duration estimate at capture** | **ABSORBED ELSEWHERE (verified)** | Fully built & committed today by duration-intelligence (`089b9ef`/`1356475`/`38d5fc2`). Both the stated-duration write *and* the estimate-when-silent step are live in `capture_fields` + `capture_generate`. **Do not build.** The duration-intelligence plan's own reconciliation section already declared this supersession; the code confirms it. |
| **2 — dedup-notes digest** | **BUILD AS WRITTEN** | Verified the read contract against real data: capture records carry `source="capture"`, `success`, and `structural.dedup_notes` whose elements have exactly `{name, action, note}` — matching `load_dedup_entries` line-for-line. Only nuance: the digest is thin (24 capture records ever, few flagged) — correct, just sparse. No blocker. |
| **3 — engagement-change capture** | **BUILD AS WRITTEN — but resolve the scheduling open-item first** | The snapshot-diff mechanism is sound and untouched by the audits; the name-based `Engagement` lookup it uses is now *doubly* confirmed correct (duration-intelligence reads `Duration source` by the same `daily_plan.py:64` display-name convention). The plan's own open item — *nothing schedules `check_for_corrections()`* — is now more acute: June works in the overlay and rarely opens Anytype to change Engagement by hand, so without a scheduled diff (piggybacked on the memory pass or morning push) this log may stay empty. Decide the schedule before relying on the data. |
| **4 — duration-bias EMA loop** | **BUILD the standalone math AS WRITTEN; wiring stays BLOCKED-ON** | The EMA math (Steps 1–3) is buildable and correct now. The `Duration source` label shipped today is exactly the signal a future bias loop needs — it tells the loop which durations are `estimated` (may correct) vs `stated` (must never touch); duration-intelligence's OQ1 even reserved a future `corrected` value for this. **Wiring remains blocked** on: (a) Phase 9 extraction (raw `signal_log` text → `{estimated, actual}` pairs) — still doesn't exist; (b) no completion-event log exists, so there is no "actual" to compare yet; (c) the unresolved project-vs-task-type keying question. All three are the same deferrals the plan already names — none newly invalidated, but the wiring is further out than "just needs data." |
| **5 — capacity-at-surfacing logging** | **BUILD WITH NAMED ADAPTATION** | The task is still valid and the interpretation-cut still right (reinforced: completion events aren't logged anywhere, so the cut completion-rate loop literally has no data source). **Adaptation:** Step 3's "Replace…" quotes an *older* `surface_log.py`. The file gained the shared staleness resolver today (commit `3468499`). The `log_surfaced` / `log_surfaced_batch` functions it edits are unchanged (lines 17–36) and still match — apply the `capacity` thread to those two functions only, leave the new resolver section (lines ~39–97) untouched. Also re-verify the `plan_generate.py` call site in Step 4 still matches before editing (it may have shifted). Mechanical, not a redesign. |

### Overall launch recommendation — LAUNCH WITH ADAPTATIONS

Build Tasks 2, 4 (math only), and 5, drop Task 1 (it shipped today inside duration-intelligence), and treat Task 3 as buildable-but-decide-its-schedule-first — no redesign is needed because the plan's central bet, *record raw signal now and interpret it in a later design pass*, is exactly what the last two days of audits vindicated. The one task the findings invalidate (Task 1) was already superseded by work that landed today, and the only real edit the findings force is a mechanical re-point of Task 5 onto a `surface_log.py` that changed under it. The heterogeneous eight-log integration cost is real but belongs to the deferred interpretation layer, not to this logging pass — so it does not gate this launch.
