...
- **Hobby / Wellbeing-side work renders as an open-ended, self-directed block she fills — NOT specific tasks the system picks. [BUILT 2026-07-02.]** June: "hobbies should be framed as open-ended scheduling blocks, so I get to choose myself — this is better than the system choosing for me, poorly." This is the addendum's *never-pick-her-threads* / linear-vs-nonlinear principle, mapped onto the **Side field**. Implemented in `daily_plan.partition_by_side` + `open_hobby_block`, wired into both `build_context` (live) and `run()`: Wellbeing-side tasks (a linked project is Wellbeing and none is Obligation) collapse into one "Fun / hobby time — your choice" block instead of being scheduled individually; the Wellbeing *projects* still show in context so she sees the threads. Obligation-side non-foreground work is still backgrounded and scheduled where it fits. **Prerequisite to see it live:** projects must be tagged `Side = Wellbeing` (the field is new; a tagging pass is owed). *Possible refinements (not built): block duration is a fixed 60 min; a foregrounded hobby project still collapses to the open block rather than a per-project one; conditional emission on very fragmented days.*
