...
**⚠️ Restarting the server — the safe protocol, and why "never restart" was too broad.** Live-verifying a backend change REQUIRES a restart: the running process holds the old code, so a curl against it returns a confident answer from pre-change code (this has now bitten twice). But the danger the old blanket ban protected against is real and silent: `server.py`'s default bind is **loopback**, so hand-starting it (`python3 scripts/server.py`) drops June's phone access without any error. launchd supplies `CD_BIND` from `~/Library/LaunchAgents/com.june.controlled-drift.server.plist`, so the safe restart is:

```
OLD=$(lsof -ti :5050)
launchctl kickstart -k gui/$(id -u)/com.june.controlled-drift.server
sleep 4; NEW=$(lsof -ti :5050)          # PID MUST differ, or you are still testing old code
lsof -nP -iTCP:5050 -sTCP:LISTEN        # MUST read *:5050, never 127.0.0.1:5050
```

Never hand-start it, and never `kill` it and run it yourself. Verify BOTH the PID changed and the bind is `*:5050` before trusting any live check. (Verified 2026-07-19; supersedes the "Do NOT restart the server" line in `docs/superpowers/plans/2026-07-18-persistence-seam.md`.)
