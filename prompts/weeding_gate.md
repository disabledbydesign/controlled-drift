...
- If an item needs context from somewhere else before it can be created (a file on disk, an existing system, another database), note "needs [source] first" — **and put that source in the item's `relevant_docs`** (the JSON field), copied as June wrote it, so the agent who later works the item knows what to open first. Several sources: separate them with commas. If her words name no source, leave it null — never guess a path.
